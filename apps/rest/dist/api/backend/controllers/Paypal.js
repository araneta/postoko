...
function convertToPaypalItems(products, currencyCode) {
    ...
    return products.map(item => ({
        ...
        category: 'PHYSICAL_GOODS', // Cast to the correct type
...
export default class PaypalController {
    ...
    static async createCheckoutSession(req, res) {
        ...
        try {
            ...
            const environment = payment.paypalMode === "sandbox"
                ? new paypal.core.SandboxEnvironment(payment.paypalClientId, payment.paypalClientSecret)
                : new paypal.core.LiveEnvironment(payment.paypalClientId, payment.paypalClientSecret);
            ...
            // Calculate total
            const totalAmount = paypalItems.reduce((sum, item) => sum + parseFloat(item.unit_amount.value) * parseInt(item.quantity, 10), 0);
            // Create order
