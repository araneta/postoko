...
export default class InventoriesController {
    ...
    static async getMovementsByProduct(req, res) {
        ...
        try {
            ...
            const movements = await db.select()
                ...
                .where(and(eq(inventoryMovementsTable.productId, productId), eq(inventoryMovementsTable.storeInfoId, storeInfoId)))
    ...
    static async getMovementsByStore(req, res) {
        const auth = getAuth(req);
        if (!auth.userId) {
            return res.status(401).send('Unauthorized');
        }
        ...
        try {
            ...
            if (type) {
                whereCondition = and(whereCondition, eq(inventoryMovementsTable.type, type));
            }
            // Get movements
            const movements = await db.select({
    ...
    static async getLowStockProducts(req, res) {
        ...
        try {
            ...
            const lowStockProducts = await db.select({
                ...
                stockDeficit: sql `${productsTable.minStock} - ${productsTable.stock}`
            })
                .from(productsTable)
                .where(and(eq(productsTable.storeInfoId, storeInfoId), sql `${productsTable.stock} <= ${productsTable.minStock}`))
    ...
    static async getStockSummary(req, res) {
        ...
        try {
            ...
            const [summary] = await db.select({
                ...
                totalQuantity: sql `SUM(${productsTable.stock})`,
                totalValue: sql `SUM(${productsTable.stock}::numeric * ${productsTable.price}::numeric)`,
                lowStockCount: sql `COUNT(CASE WHEN ${productsTable.stock} <= ${productsTable.minStock} THEN 1 END)`,
                outOfStockCount: sql `COUNT(CASE WHEN ${productsTable.stock} = 0 THEN 1 END)`
            })
                .from(productsTable)
            ...
            const [movementSummary] = await db.select({
                totalSales: sql `SUM(CASE WHEN ${inventoryMovementsTable.type} = 'sale' THEN ABS(${inventoryMovementsTable.quantity}) ELSE 0 END)`,
                totalPurchases: sql `SUM(CASE WHEN ${inventoryMovementsTable.type} = 'purchase' THEN ${inventoryMovementsTable.quantity} ELSE 0 END)`,
                totalReturns: sql `SUM(CASE WHEN ${inventoryMovementsTable.type} = 'return' THEN ABS(${inventoryMovementsTable.quantity}) ELSE 0 END)`,
                totalAdjustments: sql `SUM(CASE WHEN ${inventoryMovementsTable.type} = 'adjustment' THEN ${inventoryMovementsTable.quantity} ELSE 0 END)`
            })
                .from(inventoryMovementsTable)
                .where(and(eq(inventoryMovementsTable.storeInfoId, storeInfoId), gte(inventoryMovementsTable.createdAt, todayStart), lte(inventoryMovementsTable.createdAt, todayEnd)));
            res.status(200).json({
                stock: {
