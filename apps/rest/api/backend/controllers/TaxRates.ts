...
export default class TaxRatesController {
    // Create tax rate
    static async createTaxRate(req: Request, res: Response) {
        ...
        try {
            ...
            if (isDefault) {
                await db.update(taxRatesTable)
                    .set({ isDefault: false })
                    .where(and(
                        eq(taxRatesTable.storeInfoId, storeInfoId),
                        eq(taxRatesTable.isDefault, true)
                    ));
            }
    ...
    static async getTaxRates(req: Request, res: Response) {
        const auth = getAuth(req);
        if (!auth.userId) {
            return res.status(401).send('Unauthorized');
        }
    ...
    // Get tax rate by ID
    static async getTaxRateById(req: Request, res: Response) {
        const auth = getAuth(req);
        if (!auth.userId) {
            return res.status(401).send('Unauthorized');
        }
        ...
        try {
            ...
            const taxRate = await db.select()
                .from(taxRatesTable)
                .where(and(
                    eq(taxRatesTable.id, parseInt(id)),
                    eq(taxRatesTable.storeInfoId, storeInfoId)
                ));
    ...
    // Update tax rate
    static async updateTaxRate(req: Request, res: Response) {
        const auth = getAuth(req);
        if (!auth.userId) {
            return res.status(401).send('Unauthorized');
        }
        ...
        try {
            ...
            if (rate !== undefined && rate !== null) updateData.rate = parseFloat(rate.toString());
    ...
    // Delete tax rate
    static async deleteTaxRate(req: Request, res: Response) {
        const auth = getAuth(req);
        if (!auth.userId) {
            return res.status(401).send('Unauthorized');
        }
    ...
    static async getDefaultTaxRate(req: Request, res: Response) {
        const auth = getAuth(req);
        if (!auth.userId) {
            return res.status(401).send('Unauthorized');
        }
    ...
    static async assignTaxRateToProduct(req: Request, res: Response) {
        const auth = getAuth(req);
        if (!auth.userId) {
            return res.status(401).send('Unauthorized');
        }
        ...
        try {
            ...
            const taxRate = await db.select()
                .from(taxRatesTable)
                .where(and(
                    eq(taxRatesTable.id, parseInt(taxRateId)),
                    eq(taxRatesTable.storeInfoId, storeInfoId)
                ));
